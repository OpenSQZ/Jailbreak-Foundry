...
# Attack Implementation Agent (No-Code Variant)
...
**IMPORTANT: This is a "no-code" variant - you do NOT have access to any official GitHub repositories or reference implementations. You must rely solely on the implementation plan derived from the paper.**
...
**Audit Verdict** (if refinement mode): `attacks_paper_info/no_code/{arxiv_id}/Implementation_verdict.md`
...
3) **No-code variant**: You do NOT have access to any cloned repository or reference code. Implement purely from the implementation plan.
...
- If the paper/plan specifies multi-attempt criteria (e.g., 3/3 or 2/3), expose any needed controls as `AttackParameter`s and implement attempt-aware behavior accordingly.
...
class {MethodName}Attack(ModernBaseAttack):
    NAME = "method_name_gen_nocode"  # MUST append "_gen" suffix
...
**Requirements**:
- File location: `src/jbfoundry/attacks/generated/{NAME}_nocode.py`
...
- If the paper or plan includes **attack search control** (e.g., `n_restarts`, `n_attempts`, retries, beam restarts, sampling loops that materially affect success), those are **algorithmic**, not evaluation.
...
Create/update `attacks_paper_info/no_code/{arxiv_id}/coverage_analysis.md` based on the implementation plan:
...
Create `scripts/comprehensive_tests/attack/no_code/test_{attack_name}_comprehensive.sh` that calls test_comprehensive.py:

```bash
#!/bin/bash

# Configuration
ATTACK_NAME="{attack_name}_gen_nocode"
SAMPLES="1"
...
```bash
bash scripts/comprehensive_tests/attack/no_code/test_{attack_name}_comprehensive.sh
```
...
1) Read `attacks_paper_info/no_code/{arxiv_id}/Implementation_verdict.md`.
2) Read `src/jbfoundry/attacks/generated/{attack_name}_nocode.py`.
...
- [ ] Did NOT reference or use any cloned repository code
...
```json
{
  ...
  "result": {
    ...
    "implementation_file": "src/jbfoundry/attacks/generated/attack_name_nocode.py",
    "test_script": "scripts/comprehensive_tests/attack/no_code/test_attack_name_comprehensive.sh",
    "completed": true
  }
}
```
