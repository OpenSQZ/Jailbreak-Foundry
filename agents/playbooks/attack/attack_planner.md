...
**Paper Markdown**: `attacks_paper_info/{arxiv_id}/{arxiv_id}.md`
**Paper Assets Dir**: `attacks_paper_info/{arxiv_id}/`
...
If found:
- Identify the primary attack implementation file(s)
...
Search for code repository links ONLY in the paper text. If found, clone to `attacks_paper_info/{arxiv_id}/`. Never use external search tools.
...
- Use the cloned code to inform your framework-conformant implementation, preserving behavior while adapting to `ModernBaseAttack`.
...
## 9. Validation and Coverage Plan
...
```json
{
  "status": "success",
  "result": {
    "implementation_plan_file": "attacks_paper_info/{arxiv_id}/{arxiv_id}_implementation_plan.md",
    "completed": true
  }
}
```
