...
Implementation Notes (Refinement #1):
=====================================
Addressed gaps from implementation_gap_notes.md:

FIXED in this file:
- Gap #1: System prompt now included in returned message history (lines 486, 493)
- Gap #3: First turn query no longer uses max_tokens constraint (line 470)
- Gap #9: System message maintained throughout conversation (lines 486, 493)
- Gap #2: Default rewrite model updated to exact paper version gpt-4o-2024-05-13 (line 35)

CANNOT BE FIXED in this file (framework-level issues):
- Gap #4: Rewrite iteration count - Framework calls generate_attack once per attempt,
  not multiple times within a single call. The paper's rewrite_iteration=1 is effectively
  matched by our single-pass design.
- Gap #8: Error handling and retry logic - The framework's LLMLiteLLM already implements
  retry logic with max_retries=5 (litellm.py lines 369-402), so adding redundant retries
  here would be incorrect.
- Gap #10: Judge system - Framework uses its own judge (evaluator), cannot be changed
  in attack implementation. Judge differences are expected between frameworks.
- Gap #11: Context model option - This would require framework changes to support
  a separate model for first turn vs second turn. Not implementable in attack code alone.
- Gap #12: N_restarts logic - Framework handles restarts by calling generate_attack
  multiple times (via --attempts-per-query). Each call generates a fresh outline due
  to temperature=1.0, providing the same diversity as the paper's approach.

MINOR DIFFERENCES (no ASR impact expected):
- Gap #5: Clean conversation logic is equivalent, just implemented differently
- Gap #6: Follow-up prompt wording is identical to reference
- Gap #7: Temperature settings match exactly (1.0 for all calls)
"""
...
class AirGen(ModernBaseAttack):
    ...
    PARAMETERS = {
        ...
        "rewrite_model": AttackParameter(
            ...
            description="Model used to generate the AIR outline (paper uses gpt-4o-2024-05-13)",
        ...
        "target_max_tokens": AttackParameter(
            ...
            param_type=int,
            default=300,
            description="Max tokens for second turn refinement (first turn is unconstrained per paper)",
...
"""
    ...
    def generate_attack(self, prompt: str, goal: str, target: str, **kwargs) -> List[Dict[str, str]]:
        ...
        - Gap #1 & #9: System prompt is now included in the returned message history
          to match reference implementation (models.py lines 12-14, 72-75)
        - Gap #3: First turn query does NOT use max_tokens to allow full paper draft generation.
          The reference implementation intentionally comments out max_tokens on line 82 of models.py.
          Only the second turn (refinement) uses max_tokens=300.
        """
        # Get parameters
        ...
        # Gap #3 fix: Do NOT use max_tokens for first turn to allow complete paper draft
        # Reference: models.py line 82 (commented out max_tokens)
        ...
        # First turn: no max_tokens constraint (reference implementation line 82)
        response_1 = target_llm.query(turn1_messages, temperature=1.0)
        ...
        # Gap #1 & #9 fix: Include system message in returned conversation
        # Reference: models.py lines 72-75, 93-94 maintain system message throughout
